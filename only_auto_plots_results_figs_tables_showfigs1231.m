...
% This file generates the main plot results. 
%  ACF and trace plots 
%  BIC and logL distributions 
% without rerunning the main loop.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Step 1: main_plot_result   
disp(['Will start drawing results, sag1 is ', num2str(sag1), ' ___ sa is ', num2str(sa)]);

sourcefile = strcat('allmcmcresults/', countryText, '_mcmc_res_', '.csv');
...
imwrite(imageData11, fullfile('allmcmcresults', strcat(countryText, '_results_', '.png')));  
...
if exist(destfile, 'file')
    delete(destfile);
    disp(['Deleted file ', destfile, ' to prepare for the next plot.']);
else
    disp(['File to delete ', destfile, ' does not exist.']);
end

% Rename the table results to avoid overwriting in the loop
sourcefile2 = 'allmcmcresults/main_plot_results_SZ_epifrom1118.csv'; 
destfile2 = strcat('allmcmcresults/main_plot_results_SZ_epifrom1118_', '.csv');
...
if exist(sourcefile2, 'file')
    delete(sourcefile2);
    disp(['Deleted file ', sourcefile2]);
else
    disp(['File to delete ', sourcefile2, ' does not exist.']);
end

%%%%%%%%%%%%%%%%%%%%%%%%%% Step 2: BIC
sourcefile31 = strcat('allmcmcresults/', countryText, '_log_likelihood_', '.csv');
...
imwrite(imageData31, fullfile('allmcmcresults', strcat(countryText, '_BIC_logL_', '.png')));  
...
if exist(destfile31, 'file')
    delete(destfile31);
    disp(['Deleted file ', destfile31, ' to prepare for the next plot.']);
else
...
destfile32 = strcat('allmcmcresults/BIC_all_', '.csv');
...
destfile33 = strcat('allmcmcresults/BIC_withoutburnin_all_', '.csv');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Step 3: Draw posterior distributions in a loop
disp(['Will start drawing posterior distributions, sag1 is ', num2str(sag1), ' ___ sa is ', num2str(sa)]);

sourcefile41 = strcat('allmcmcresults/', countryText, '_mcmc_res_', '.csv');
...
imwrite(imageData41, fullfile('allmcmcresults', strcat(countryText, '_PosteriorDistri_', '.png')));  
...
sourcefile51 = strcat('allmcmcresults/', countryText, '_mcmc_res_', '.csv');
...
imwrite(imageData51, fullfile('allmcmcresults', strcat(countryText, '_ACF_', '.png')));  
...
sourcefile71 = strcat('allmcmcresults/', countryText, '_log_likelihood_', '.csv');
...
imwrite(imageData61, fullfile('allmcmcresults', strcat(countryText, '_traceplots_withinlogL_', '.png')));  
disp(['Finished drawing trace plots, sag1 is ', num2str(sag1), ' ___ sa is ', num2str(sa)]);
